    ...
    <>
        ...
        >
            ...
            <DialogActions>
                ...
                onClick={async () => {
                    try {
                    await fetch(`/api/journeys/${journeyToDelete}`, { method: "DELETE" });
                    setLocalJourneys(localJourneys.filter(j => j.id !== journeyToDelete));
                    } catch (err) {
...
export async function getServerSideProps() {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/journeys`);
