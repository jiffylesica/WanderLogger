...
export default async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        try {
            ...
            const {journey_id, pin_title, pin_description, latitude, longitude, date_traveled, image_url} = req.body;

            const [newPin] = await db('pins').insert({journey_id, pin_title, pin_description, latitude, longitude, date_traveled, image_url}).returning('*');
