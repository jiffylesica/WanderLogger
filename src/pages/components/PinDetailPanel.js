import {Drawer, Stack, Typography, IconButton, TextField, Button} from "@mui/material";
...
export default function PinDetailPanel ({
    ...
    const handleSave = async () => {
        ...
        setPins((prev) => 
            ...
            prev.map((pin) =>
                pin.latitude === activePin.latitude && pin.longitude === activePin.longitude
                    ? activePin : pin
            )
        );
        setIsPinPanelOpen(false);
        ...
        >
            <Stack spacing={2} sx={{width: "50vw", padding:3}}>
                ...
                {isEditMode ? (
                    ...
                    <>
                        <Typography variant="subtitle1"><strong>Title:</strong> {activePin?.title}</Typography>
                        <Typography variant="body2"><strong>Description:</strong> {activePin?.description || "—"}</Typography>
                        <Typography variant="body2"><strong>Date:</strong> {activePin?.date}</Typography>
                        <Typography variant="body2"><strong>Image URL:</strong> {activePin?.image_url || "—"}</Typography>
